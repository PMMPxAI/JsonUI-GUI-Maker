// ==========================================================
// import.js — import .json / .mcpack / .zip files; library list
// ==========================================================
window.SW = window.SW || {};
...
(function (SW) {
  ...
  async function handleFile(file) {
    ...
    if (name.endsWith('.json')) {
      ...
      try {
        ...
        // Fallback: any JSON file in the zip
        if (!jsonFiles.length) {
          zip.forEach((path, entry) => {
        ...
        SW.toast('Lecture impossible: ' + e.message, 'error');
      }
    } else {
  ...
  function renderZipPicker(zipName, items) {
    ...
    for (const it of items) {
      ...
      action.addEventListener('click', async () => {
        try {
          ...
          SW.toast('Erreur: ' + e.message, 'error');
        }
      });
  ...
  function tryLoadJson(text, source) {
    let parsed;
    try { parsed = JSON.parse(text); }
    catch (e) { SW.toast('JSON invalide: ' + e.message, 'error'); return; }
      ...
      for (const pack of packs) {
        ...
        for (const it of items) {
          ...
          action.addEventListener('click', async (e) => {
            ...
            } catch (err) { SW.toast('Erreur: ' + err.message, 'error'); }
          });
